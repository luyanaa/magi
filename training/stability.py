...
L2: pointwise degeneracy residual projection + Casimir soft projection
...
L6: EMA monitoring of observable control proxies (every 1000 steps)
L7: Global energy-like field audit (every epoch)

Auto-rollback triggers:
- Negative signed-work ratio > 1% over 1000 steps, when explicitly supplied
...
from ..runtime.device_utils import get_device, get_device_type, autocast_context
...
class QFactorMonitor:
    ...
    Schedule: same cadence as trajectory diagnostics (every step, logged every 100).
...
class AtaxiaCatalepsyMonitor:
    ...
    Schedule: every step, logged every 100, matching trajectory diagnostics.
    """
...
class StabilityMonitor:
    ...
    def __init__(
        ...
        negative_work_threshold: float = 0.01,
        ...
        self.window_size = window_size
        self.negative_work_threshold = negative_work_threshold
        ...
        self.negative_work_history: List[float] = []
    ...
    def update(self, metrics: Dict[str, float]):
        """Append explicitly named diagnostics from the current step."""
        if "negative_work_ratio" in metrics:
            self.negative_work_history.append(metrics["negative_work_ratio"])
        if "E" in metrics:
        ...
        for hist in [self.negative_work_history, self.energy_history, self.router_entropy_history, self.q_history, self.ataxia_history, self.catalepsy_history]:
    ...
    def check_triggers(self, step: int) -> Tuple[bool, Optional[str]]:
        ...
        Returns:
            ...
            reason: str or None
        """
        # Trigger 1: signed-work negative ratio > threshold over the window.
        # This is a rollout diagnostic, not an entropy-production estimate.
        if len(self.negative_work_history) >= self.window_size:
            neg_ratio = sum(self.negative_work_history[-self.window_size:]) / self.window_size
            if neg_ratio > self.negative_work_threshold:
                reason = f"Negative work ratio {neg_ratio:.4f} > {self.negative_work_threshold}"
                self.trigger_log.append({"step": step, "reason": reason, "type": "negative_work"})
                return True, reason
    ...
    def get_report(self) -> Dict:
        ...
        valid_catalepsy = [c for c in self.catalepsy_history if not math.isnan(c)]
        return {
            "negative_work_history_len": len(self.negative_work_history),
...
class AutoRollback:
    ...
    def __init__(self, checkpoint_dir: str = "./checkpoints",
                 streak_reset_steps: int = 5000):
        self.checkpoint_dir = checkpoint_dir
        self.level = 0
        self.consecutive_triggers = 0
        self.streak_reset_steps = int(streak_reset_steps)
    ...
    def on_trigger(
        ...
        """
        # Triggers separated by a long healthy stretch are independent events.
        # Without this decay the counter only ever grows, so a long run is
        # eventually pinned at level 4 regardless of recovery.
        if (self.last_trigger_step is not None
                and step - self.last_trigger_step > self.streak_reset_steps):
            self.consecutive_triggers = 0
    ...
    def _level1_adjust(self, model: nn.Module, optimizer: torch.optim.Optimizer) -> Dict:
        """Level 1: escalate the balance weight and sharpen routing.

        The returned values are applied by the caller (``BrainMoETrainer``);
        this method only decides them, so the rollback stays testable without
...
def selective_fp32_forward(fn, *args, **kwargs):
    ...
    for i in range(1200):
        monitor.update({
            "negative_work_ratio": 0.02 if i > 500 else 0.0,
            "E": 1.0 + i * 0.01,
            "router_entropy": 0.05,
        })
